...
def direct_prompting(api_key, metamodel_image_path, mockup_image_path, first_example_code_path, second_example_code_path, metamodel_text_path, structural_model_path):
    ...
    direct_prompt += "2. I will also provide you with an image of the GUI metamodel and a description file for it.\n"
    direct_prompt += "3. Your goal is to generate Python code that accurately represents all GUI elements in the mock-up.\n"
    direct_prompt += "4.Please provide the Python code representing the GUI elements only, without any introductory or formatting lines.\n"
    direct_prompt += "5. It is crucial: the structure of the Python code you generate should be similar to the code samples provided as examples, while the content should be derived from the UI mockups you receive.\n"
    direct_prompt += "6. The generated code should adhere to the GUI metamodel.\n"
    direct_prompt += "7. Incorporate the provided examples and ensure the generated code aligns with the user's expectations.\n"
    direct_prompt += "8. Once you're ready, I will present you with a UI mock-up image, and you need to generate Python code for its GUI elements.\n\n"
    ...
    python_code = gpt4o_call(api_key, mockup_image_path, base64_metamodel, base64_mockup, direct_prompt, first_example_code_path, second_example_code_path, metamodel_text_path, structural_model_path)
...
def run_pipeline_gui_generation(mockup_image_path):
    # Start time
    start_time_gui_model = time.time()

    global gui_output_dir  # Ensure we use the imported variable, not a local one
