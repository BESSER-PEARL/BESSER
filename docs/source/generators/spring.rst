...
- **Entities**: each class becomes a JPA ``@Entity`` with an ``@Table``, getters and
  ...
  auto-generate a ``String`` identifier. A class hierarchy that marks no identifier gets one:
  its root class receives an ``id`` of type ``int``, generated by the database (an existing
  attribute named ``id`` is promoted instead). The input model is not modified.
  An abstract class becomes a ``@MappedSuperclass``
