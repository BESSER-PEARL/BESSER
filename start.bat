...
if exist "besser_venv\Scripts\activate.bat" (
    REM Set up the PowerShell command to activate the virtual environment, set PYTHONPATH, and install requirements
    set "psCommand=cd '%PROJECT_PATH%'; . .\besser_venv\Scripts\Activate.ps1; $env:PYTHONPATH='%PROJECT_PATH%'; if (Test-Path requirements.txt) { pip install -r requirements.txt };"

    REM Launch a new PowerShell session with the environment activated, bypassing execution policy
    powershell -NoExit -ExecutionPolicy Bypass -Command "%psCommand%"
