...
async def personalize_gui_page(payload: Dict[str, Any] = Body(...)):
    ...
    system_prompt = (
        ...
        "You also receive a user profile model. Your job is to adapt the frontend to this user, "
        "making decisions based on the information in the profile. Do NOT change the semantics of "
        "the page (its meaning, message, and purpose stay the same) — change only its style and "
        "presentation.\n\n"
        "STRICT OUTPUT RULES: "
        ...
        "3) You MAY change class lists and the 'style' objects in both components and css rules to "
        "adapt style and presentation to the user profile. Every other value must be byte-for-byte "
        "identical to the input. "
        "3a) Do NOT change the semantics: preserve the visible text content, attribute label "
        "values (placeholder, title, alt, link text), and the page's message and purpose. A visual "
        "adaptation such as enlarging the font is made ENTIRELY through 'style'/class changes; "
        "never rewrite text into a description of the adaptation (e.g. do NOT turn a heading into "
    ...
    user_prompt = (
        ...
        "Make decisions based on the information in the profile. "
        "Only change something if you feel that the user profile gives a clear reason to adapt the page's style and presentation. " 
        "You can assume that the profile contains the most important information about the user and anything that is missing can be assumed that the user does not have that characteristic." 
        ". Adapt the page's style and "
        "presentation to this user; do not change its core semantics. Presentation refers also to textual changes, " 
        "like making text more concise or verbose," 
        "or changing the tone of voice, language, etc. "
