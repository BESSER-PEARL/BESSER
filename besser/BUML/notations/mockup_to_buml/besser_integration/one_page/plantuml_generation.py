...
from besser.BUML.notations.mockup_to_buml.utilities.file_utils import read_file_contents
import retrying
import requests
...
def direct_prompting_image_to_plantuml(api_key, mockup_image_path, first_gui_model_path, second_gui_model_path, third_gui_model_path, forth_gui_model_path, plantuml_code_example_path):
    ...
    plantuml_code = gpt4o_call_plantuml(api_key, direct_prompt, mockup_image_path, first_gui_model_path, plantuml_code_example_path)
...
def run_pipeline_plantuml_generation(mockup_image_path: str):
    # Use output_file_name from the imported config file
    global output_file_name  # Ensure we use the imported variable, not a local one

    # Generate the code using the direct prompt method
    plantuml_code = direct_prompting_image_to_plantuml(
        api_key, mockup_image_path, first_gui_model_path, second_gui_model_path,
        third_gui_model_path, forth_gui_model_path, plantuml_code_example_path
    )
...
if __name__ == "__main__":
    # Replace this with the actual path or logic to provide the mockup_image_path
    mockup_image_path = "path_to_mockup_image"
