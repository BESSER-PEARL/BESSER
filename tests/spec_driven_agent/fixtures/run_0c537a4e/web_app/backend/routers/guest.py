...
def search_guest(
    ...
    if emailAddress is not None:
        query = query.filter(Guest.emailAddress.ilike(f"%{emailAddress}%"))
    ...
    if phoneNumber is not None:
        query = query.filter(Guest.phoneNumber.ilike(f"%{phoneNumber}%"))
...
async def create_guest(guest_data: GuestCreate, database: Session = Depends(get_db)) -> Guest:
    ...
    db_guest = Guest(
        firstName=guest_data.firstName,        familyName=guest_data.familyName,        phoneNumber=guest_data.phoneNumber,        emailAddress=guest_data.emailAddress        )
...
async def update_guest(guest_id: int, guest_data: GuestCreate, database: Session = Depends(get_db)) -> Guest:
    ...
    setattr(db_guest, 'phoneNumber', guest_data.phoneNumber)
    setattr(db_guest, 'emailAddress', guest_data.emailAddress)
