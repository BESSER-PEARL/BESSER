...
nn_model.add_layer(Conv2D(name="l1", activation_function="relu", in_channels=3, out_channels=6, kernel_height=5, 
                          kernel_width=5, stride_height=1, stride_width=1, padding_amount=0))
nn_model.add_layer(PoolingLayer(name="l2", activation_function=None, pooling_type="average", dimension="2D", 
                                kernel_height=2, kernel_width=2, stride_height=2, stride_width=2))

nn_model.add_layer(Conv2D(name="l3", activation_function="relu", in_channels=6, out_channels=16, kernel_height=5, 
                          kernel_width=5, stride_height=1, stride_width=1, padding_amount=0))
nn_model.add_layer(PoolingLayer(name="l4", activation_function=None, pooling_type="average", dimension="2D", 
                                kernel_height=2, kernel_width=2, stride_height=2, stride_width=2))

nn_model.add_layer(FlattenLayer(name="l5", activation_function=None))
nn_model.add_layer(LinearLayer(name="l6", activation_function="relu",  in_features=12544, out_features=120))
nn_model.add_layer(LinearLayer(name="l7", activation_function="relu", in_features=120, out_features=84))
nn_model.add_layer(LinearLayer(name="l8", activation_function="softmax", in_features=84, out_features=2))
