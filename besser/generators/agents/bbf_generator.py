...
class BBFGenerator(GeneratorInterface):
    ...
    the agent code, using the BESSER Agent Framework (BBF), based on an input agent model.
    ...
    def generate(self):
        """
        Generates the BBF agent code and saves it to the specified output directory.
        ...
        agent_template = env.get_template('bbf_bot_template.py.j2')
        agent_path = self.build_generation_path(file_name=f"{self.model.name}.py")
        with open(agent_path, mode="w") as f:
            generated_code = agent_template.render(bot=self.model)
