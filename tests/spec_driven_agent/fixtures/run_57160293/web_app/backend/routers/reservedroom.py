...
def search_reservedroom(
    ...
    if extraCharges is not None:
        query = query.filter(ReservedRoom.extraCharges == extraCharges)
...
async def create_reservedroom(reservedroom_data: ReservedRoomCreate, database: Session = Depends(get_db)) -> ReservedRoom:
    ...
    db_reservedroom = ReservedRoom(
        extraCharges=reservedroom_data.extraCharges,        agreedPrice=reservedroom_data.agreedPrice,        room_id=reservedroom_data.room,        booking_id=reservedroom_data.booking        )
...
async def bulk_create_reservedroom(items: list[ReservedRoomCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_reservedroom = ReservedRoom(
                extraCharges=item_data.extraCharges,                agreedPrice=item_data.agreedPrice,                room_id=item_data.room,                booking_id=item_data.booking            )
            database.add(db_reservedroom)
...
async def update_reservedroom(reservedroom_id: int, reservedroom_data: ReservedRoomCreate, database: Session = Depends(get_db)) -> ReservedRoom:
    ...
    setattr(db_reservedroom, 'extraCharges', reservedroom_data.extraCharges)
...
async def delete_reservedroom(reservedroom_id: int, database: Session = Depends(get_db)):
    ...
    if db_reservedroom is None:
        raise HTTPException(status_code=404, detail="ReservedRoom not found")
    related_booking = db_reservedroom.booking
    for related in (list(related_booking) if isinstance(related_booking, list) else [item for item in [related_booking] if item is not None]):
        remaining = len(related.reservedRooms) - 1
        if remaining < 1:
            raise HTTPException(status_code=409, detail="Cannot delete ReservedRoom " + str(reservedroom_id) + ": Booking " + str(getattr(related, 'id')) + " requires at least 1 reservedRooms")
