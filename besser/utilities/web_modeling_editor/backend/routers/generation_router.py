...
async def personalize_gui_page(payload: Dict[str, Any] = Body(...)):
    ...
    system_prompt = (
        ...
        "Your job is NOT to redesign the page. It is to make the SMALLEST set of changes that a "
        "SPECIFIC, CONCRETE attribute in the user profile actually requires — and nothing else. "
        "The default outcome is to return the page UNCHANGED. Only deviate from the original when "
        "an attribute in the profile creates a real, demonstrable accessibility or comprehension "
        "need for THIS user.\n\n"
        "DECISION PROCESS (apply silently, then output only the JSON):\n"
        "1) For each attribute in the user profile, ask: 'Does this attribute imply a concrete UI "
        "barrier on THIS page?' Most attributes do not. An attribute justifies a change only if "
        "there is a direct, well-established causal link between it and a UI adaptation.\n"
        "2) If an attribute has no such link, IGNORE it. Do not invent a need. Demographic, "
        "cultural, religious, gender, name, or location attributes almost never justify visual or "
        "textual changes on their own — leave the page as-is for them.\n"
        "3) Only when a link exists, apply the minimal adaptation that resolves that specific need, "
        "and touch only the components/styles relevant to it. Leave everything else identical.\n\n"
        ...
        "- Stated reading level below the page's wording (e.g. B1 English on advanced copy) -> "
        "simplify the wording while keeping meaning; do NOT translate unless a different language "
        "is explicitly the user's language.\n"
        ...
        "- A stated preferred language different from the page's language -> translate the visible "
        "text into that language.\n\n"
        "EXAMPLES OF NON-JUSTIFIED (return unchanged for these): a user who reads English fluently "
        "on an English page; religion, ethnicity, gender, age alone, job title, hobbies, or any "
        "attribute with no direct UI consequence. NEVER map an irrelevant attribute to a styling "
        "change (e.g. do not change font size because of someone's religion or nationality).\n\n"
        "STRICT OUTPUT RULES: "
        ...
        "but ONLY for the specific needs you justified above. Every other value must be byte-for-byte "
        "identical to the input. "
        "4) Keep every css rule's 'selectors' and 'pageId' values exactly as given. "
        "5) If no profile attribute justifies any change, return the input page verbatim. "
        "6) The result must be valid JSON, parseable as-is, and remain a working GrapesJS page."
    )
    user_prompt = (
        f'Personalize the GUI page named "{page_name}" for the following user — but only if the '
        "profile actually warrants it.\n\n"
        f"User profile document:\n{json.dumps(profile_document, ensure_ascii=False, indent=2)}\n\n"
        "GUI page to adapt (GrapesJS):\n"
        f"{json.dumps({'components': components, 'css': css}, ensure_ascii=False)}\n\n"
        "First, silently identify which (if any) profile attributes create a concrete UI need for "
        "this user. Change ONLY what those needs require and leave every other part of the page "
        "exactly as given. If nothing in the profile warrants a change, return the page unchanged. "
