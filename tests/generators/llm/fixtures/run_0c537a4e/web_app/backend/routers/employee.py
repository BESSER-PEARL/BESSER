...
def search_employee(
    ...
    if emailAddress is not None:
        query = query.filter(Employee.emailAddress.ilike(f"%{emailAddress}%"))
    ...
    if phoneNumber is not None:
        query = query.filter(Employee.phoneNumber.ilike(f"%{phoneNumber}%"))
...
async def create_employee(employee_data: EmployeeCreate, database: Session = Depends(get_db)) -> Employee:
    ...
    db_employee = Employee(
        firstName=employee_data.firstName,        familyName=employee_data.familyName,        phoneNumber=employee_data.phoneNumber,        emailAddress=employee_data.emailAddress        )
...
async def bulk_create_employee(items: list[EmployeeCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_employee = Employee(
                firstName=item_data.firstName,                familyName=item_data.familyName,                phoneNumber=item_data.phoneNumber,                emailAddress=item_data.emailAddress            )
            database.add(db_employee)
...
async def update_employee(employee_id: int, employee_data: EmployeeCreate, database: Session = Depends(get_db)) -> Employee:
    ...
    setattr(db_employee, 'phoneNumber', employee_data.phoneNumber)
    setattr(db_employee, 'emailAddress', employee_data.emailAddress)
