...
BESSER also enables the definition of a BUML model from an image, for example, a picture of a hand-drawn model on a board. 
...
.. image:: ../../../tests/library_test/library_hand_draw.png
  :width: 600
...
To transform this image to a BUML model you can use the following code.
...
The function ``image_to_buml()`` has as parameters the image path and the OpenAI token, and returns the BUML model ``library_model``. 

Additionally (and similar to the PlantUML to BUML transformation), the file ``buml/buml_model.py`` is generated with the BUML model source code
definition. You could reuse that code to modify, enhance, and recreate your BUML model. The BUML model source code generated by the transformation 
from the ``library_hand_draw.png`` model is as follows.

.. literalinclude:: ../../../tests/library_test/buml/buml_model_from_image.py
   :language: python
   :linenos:
