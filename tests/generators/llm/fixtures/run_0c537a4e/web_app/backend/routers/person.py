...
def search_person(
    ...
    if emailAddress is not None:
        query = query.filter(Person.emailAddress.ilike(f"%{emailAddress}%"))
    ...
    if phoneNumber is not None:
        query = query.filter(Person.phoneNumber.ilike(f"%{phoneNumber}%"))
...
async def create_person(person_data: PersonCreate, database: Session = Depends(get_db)) -> Person:
    ...
    db_person = Person(
        firstName=person_data.firstName,        familyName=person_data.familyName,        phoneNumber=person_data.phoneNumber,        emailAddress=person_data.emailAddress        )
...
async def update_person(person_id: int, person_data: PersonCreate, database: Session = Depends(get_db)) -> Person:
    ...
    setattr(db_person, 'phoneNumber', person_data.phoneNumber)
    setattr(db_person, 'emailAddress', person_data.emailAddress)
