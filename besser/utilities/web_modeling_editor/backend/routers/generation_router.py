...
async def personalize_gui_page(payload: Dict[str, Any] = Body(...)):
    ...
    system_prompt = (
        ...
        "information in the profile. Do NOT change the semantics of the page (its meaning, message, "
        "and purpose stay the same) — change only its style and presentation.\n\n"
        ...
        "objects of existing css rules, and (c) change class lists and 'style' objects of "
        "individual components (Tier 2). Every OTHER value must be byte-for-byte identical to the "
        "input. "
        ...
        "adaptation such as enlarging the font is made ENTIRELY through 'style'/class/CSS-rule "
        "changes; never rewrite text into a description of the adaptation (e.g. do NOT turn a "
        "heading into 'bigger text for readability'). "
