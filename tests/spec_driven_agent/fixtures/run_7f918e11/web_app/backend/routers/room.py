...
async def create_room(room_data: RoomCreate, database: Session = Depends(get_db)) -> Room:
    ...
    db_room = Room(
        capacity=room_data.capacity,        description=room_data.description,        roomNumber=room_data.roomNumber,        standardPrice=room_data.standardPrice        )
...
async def bulk_create_room(items: list[RoomCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_room = Room(
                capacity=item_data.capacity,                description=item_data.description,                roomNumber=item_data.roomNumber,                standardPrice=item_data.standardPrice            )
            database.add(db_room)
