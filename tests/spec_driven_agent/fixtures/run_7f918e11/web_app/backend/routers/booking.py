...
async def create_booking(booking_data: BookingCreate, database: Session = Depends(get_db)) -> Booking:
    ...
    db_booking = Booking(
        departureDate=booking_data.departureDate,        commercialStatus=booking_data.commercialStatus.value,        physicalStatus=booking_data.physicalStatus.value,        arrivalDate=booking_data.arrivalDate,        employee_id=booking_data.employee,        contact_id=booking_data.contact        )
...
async def bulk_create_booking(items: list[BookingCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_booking = Booking(
                departureDate=item_data.departureDate,                commercialStatus=item_data.commercialStatus.value,                physicalStatus=item_data.physicalStatus.value,                arrivalDate=item_data.arrivalDate,                employee_id=item_data.employee,                contact_id=item_data.contact            )
            database.add(db_booking)
