...
nn_model.add_layer(EmbeddingLayer(name="l1", actv_func=None, num_embeddings=5000, embedding_dim=50))
nn_model.add_layer(DropoutLayer(name="l2", rate=0.5))
nn_model.add_layer(Conv1D(name="l3", actv_func="relu", in_channels=50, out_channels=200, kernel_dim=[4], 
                          input_reused=True, name_layer_input="l2", permute_dim=True))
nn_model.add_layer(PoolingLayer(name="l4", actv_func=None, pooling_type="max", dimension="1D", 
                                kernel_dim=[2]))
nn_model.add_layer(Conv1D(name="l5", actv_func="relu", in_channels=50, out_channels=200, kernel_dim=[5], 
...
nn_model.add_layer(LinearLayer(name="l11", actv_func="sigmoid", in_features=400, out_features=1))
