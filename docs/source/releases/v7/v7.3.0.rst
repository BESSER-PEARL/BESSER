...
* Added **UserDiagram** as a first-class diagram type for describing the different user profiles of created apps capturing personal information, language profile, competences, accessibility contraints abilities and cultural dimensions in the same metamodel space as the rest of B-UML.
...
* The BAF_generator can now ingest multiple agent models with a user profile to agent model mapping and generate an agent that provides a personalized experienced based on the available profiles
