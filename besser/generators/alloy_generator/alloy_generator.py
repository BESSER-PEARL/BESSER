...
class AlloyGenerator(GeneratorInterface):
    ...
    * A generic ``pred instance_model`` with a ``run`` command.
    ...
    def _sanitize_model_names(self) -> None:
        """Sanitize class and attribute names in-place for Alloy compatibility."""
    ...
    def _build_inheritance_and_attribute_maps(self):
        """Build inheritance, attribute, and signature maps.
    ...
    def _process_associations(self, data: dict) -> list[str]:
        """Process associations, building consistency facts and updating *data*.
    ...
    def _translate_constraints(
        ...
        """Translate OCL constraints to Alloy facts in-place.

        Returns:
            The :class:`EstadoTraductor` carrying accumulated state (e.g. date
        ...
        for constraint in self.model.constraints:
            ...
            constraint.expression = ocl_to_alloy(
                inherits_from, data, ocl_str, context, estado, enums
            )
        return estado
    ...
    def _render_spec(
        ...
        """Render all Jinja2 templates and append association facts.
