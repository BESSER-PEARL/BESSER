...
async def create_booking(booking_data: BookingCreate, database: Session = Depends(get_db)) -> Booking:
    ...
    db_booking = Booking(
        totalPrice=booking_data.totalPrice,        commercialStatus=booking_data.commercialStatus.value,        physicalStatus=booking_data.physicalStatus.value,        arrivalDate=booking_data.arrivalDate,        departureDate=booking_data.departureDate,        contact_id=booking_data.contact,        employee_id=booking_data.employee        )
...
async def bulk_create_booking(items: list[BookingCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_booking = Booking(
                totalPrice=item_data.totalPrice,                commercialStatus=item_data.commercialStatus.value,                physicalStatus=item_data.physicalStatus.value,                arrivalDate=item_data.arrivalDate,                departureDate=item_data.departureDate,                contact_id=item_data.contact,                employee_id=item_data.employee            )
            database.add(db_booking)
