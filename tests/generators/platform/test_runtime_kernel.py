...
class TestRuntimeKernel:
    ...
    def test_tick_passes_dt_and_returns_method_result(self, tmp_path, echo_model):
        ...
        value should land in the response under ``result``."""
    ...
    def test_tick_unknown_instance_raises_keyerror(self, tmp_path, counter_model):
        ...
        try:
            ...
            with pytest.raises(KeyError, match="No instance"):
                engine.tick("Counter", "nope")
        finally:
            _cleanup_imports(backend_dir)

    def test_tick_missing_method_raises_attributeerror(self, tmp_path, counter_model):
        ...
        try:
            ...
            with pytest.raises(AttributeError, match="no callable method"):
                engine.tick("Counter", inst["id"], method_name="not_a_method")
        finally:
            _cleanup_imports(backend_dir)
...
class TestPhase1Wiring:
    ...
    def test_frontend_marks_class_as_tickable(self, tmp_path, counter_model):
        out = tmp_path / "platform_tickable_flag"
        PlatformGenerator(counter_model, customization=None, output_dir=str(out)).generate()
        models_src = (out / "frontend" / "src" / "types" / "models.ts").read_text(encoding="utf-8")
        assert "hasStepMethod: true" in models_src

    def test_frontend_property_editor_renders_tick_button(self, tmp_path, counter_model):
        out = tmp_path / "platform_tick_button"
        PlatformGenerator(counter_model, customization=None, output_dir=str(out)).generate()
        editor_src = (out / "frontend" / "src" / "components" / "PropertyEditor.tsx").read_text(encoding="utf-8")
        assert "classMetadata.hasStepMethod" in editor_src
        assert "tickInstance" in editor_src

    def test_frontend_api_exports_tick_helper(self, tmp_path, counter_model):
