...
def search_bill(
    ...
    if isSettled is not None:
        query = query.filter(Bill.isSettled == isSettled)
...
async def create_bill(bill_data: BillCreate, database: Session = Depends(get_db)) -> Bill:
    ...
    db_bill = Bill(
        isSettled=bill_data.isSettled,        issuedDate=bill_data.issuedDate,        totalAmountDue=bill_data.totalAmountDue,        booking_id=bill_data.booking        )
...
async def bulk_create_bill(items: list[BillCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_bill = Bill(
                isSettled=item_data.isSettled,                issuedDate=item_data.issuedDate,                totalAmountDue=item_data.totalAmountDue,                booking_id=item_data.booking            )
            database.add(db_bill)
...
async def update_bill(bill_id: int, bill_data: BillCreate, database: Session = Depends(get_db)) -> Bill:
    ...
    setattr(db_bill, 'isSettled', bill_data.isSettled)
