...
def get_all_room(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    ...
    if detailed:
        ...
        for room_item in room_list:
            ...
            reservedroom_list = database.query(ReservedRoom).filter(ReservedRoom.room_id == room_item.id).all()
            item_dict['reservedroom'] = []
            for reservedroom_obj in reservedroom_list:
                ...
                item_dict['reservedroom'].append(reservedroom_dict)
...
def get_paginated_room(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    ...
    result = []
    for room_item in room_list:
        reservedroom_ids = database.query(ReservedRoom.id).filter(ReservedRoom.room_id == room_item.id).all()
        item_data = {
            "room": room_item,
            "reservedroom_ids": [x[0] for x in reservedroom_ids]        }
...
def search_room(
    ...
    if roomNumber is not None:
        query = query.filter(Room.roomNumber == roomNumber)
...
async def get_room(room_id: int, database: Session = Depends(get_db)) -> Room:
    ...
    reservedroom_ids = database.query(ReservedRoom.id).filter(ReservedRoom.room_id == db_room.id).all()
    response_data = {
        "room": db_room,
        "reservedroom_ids": [x[0] for x in reservedroom_ids]}
...
async def create_room(room_data: RoomCreate, database: Session = Depends(get_db)) -> Room:
    ...
    db_room = Room(
        roomNumber=room_data.roomNumber,        standardNightlyPrice=room_data.standardNightlyPrice,        description=room_data.description,        capacity=room_data.capacity        )

    database.add(db_room)
    database.flush()
    database.refresh(db_room)

    if room_data.reservedroom:
        # Validate that all ReservedRoom IDs exist
        for reservedroom_id in room_data.reservedroom:
        ...
        database.query(ReservedRoom).filter(ReservedRoom.id.in_(room_data.reservedroom)).update(
            {ReservedRoom.room_id: db_room.id}, synchronize_session=False
        )
        database.flush()
...
async def bulk_create_room(items: list[RoomCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_room = Room(
                roomNumber=item_data.roomNumber,                standardNightlyPrice=item_data.standardNightlyPrice,                description=item_data.description,                capacity=item_data.capacity            )
            database.add(db_room)
...
async def update_room(room_id: int, room_data: RoomCreate, database: Session = Depends(get_db)) -> Room:
    ...
    if room_data.reservedroom is not None:
        requested_reservedroom = set(room_data.reservedroom)
        current_reservedroom = {
            getattr(item, 'id')
            for item in database.query(ReservedRoom).filter(ReservedRoom.room_id == db_room.id).all()
        }
        reservedroom_to_detach = current_reservedroom - requested_reservedroom
        if reservedroom_to_detach:
            raise HTTPException(status_code=409, detail="ReservedRoom " + ", ".join(str(item) for item in sorted(reservedroom_to_detach)) + " requires a room: reassign it instead of removing it")
        reservedroom_to_attach = requested_reservedroom - current_reservedroom
        for reservedroom_id in reservedroom_to_attach:
            db_reservedroom = database.query(ReservedRoom).filter(ReservedRoom.id == reservedroom_id).first()
            if not db_reservedroom:
                raise HTTPException(status_code=400, detail="ReservedRoom with id " + str(reservedroom_id) + " not found")
        if reservedroom_to_attach:
            database.query(ReservedRoom).filter(ReservedRoom.id.in_(reservedroom_to_attach)).update(
                {ReservedRoom.room_id: db_room.id}, synchronize_session=False
            )
    database.commit()
    database.refresh(db_room)
...
async def delete_room(room_id: int, database: Session = Depends(get_db)):
    ...
    if db_room is None:
        raise HTTPException(status_code=404, detail="Room not found")
    related_reservedroom = db_room.reservedroom
    for related in (list(related_reservedroom) if isinstance(related_reservedroom, list) else [item for item in [related_reservedroom] if item is not None]):
        remaining = 0
        if remaining < 1:
            raise HTTPException(status_code=409, detail="Cannot delete Room " + str(room_id) + ": ReservedRoom " + str(getattr(related, 'id')) + " requires at least 1 room")
...
@router.get("/room/{room_id}/reservedroom/", response_model=None, tags=["Room Relationships"])
async def get_reservedroom_of_room(room_id: int, database: Session = Depends(get_db)):
    """Get all ReservedRoom entities related to this Room through reservedroom"""
    db_room = database.query(Room).filter(Room.id == room_id).first()
    if db_room is None:
        raise HTTPException(status_code=404, detail="Room not found")

    reservedroom_list = database.query(ReservedRoom).filter(ReservedRoom.room_id == room_id).all()

    return {
        "room_id": room_id,
        "reservedroom_count": len(reservedroom_list),
        "reservedroom": reservedroom_list
    }
