...
from tests.nn.tf_to_buml.definitions import config_list, train_param_list, \
    test_param_list, lookup_loss_functions

def parse_arguments():
    """Define and parse command-line arguments."""
    parser = argparse.ArgumentParser(
        description="Read a TF file and convert it to buml code."
    )
    parser.add_argument(
        "filename", type=str, help="Path to the TF file to read"
    )
...
def print_imports(extractor):
    ...
    for i in range(2, len(cls_to_import), 5):
        if len(cls_to_import[i:]) > 5:
            ...
            print(f"    {', '.join(map(str, cls_to_import[i:i+5]))}")
    print("from besser.generators.tf.tf_code_generator " \
          "import TFGenerator\n\n\n")
...
def main():
    ...
    f = open("tf_transformed.py", "w", encoding="utf-8")
    sys.stdout = f
