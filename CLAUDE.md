...
|-----------|------------------|-----------------------------|
| `blocker` | Python syntax errors; dependency conflicts; a Dockerfile referencing a missing file; unresolvable local imports (`missing module:`); undefined names behind a star import (`undefined name:`); an ORM module that fails to import or to configure its mappers in the subprocess smoke check (`mapper config:`); a requirement the user stated that the code does not implement (`requirement:`, from the requirements ledger — partial and unverified verdicts stay warnings); a method button bound to a table of another entity; frontend-contract violations (blank-on-load router, dead submit handler, web-app request with no frontend at all, rival framework imported into the scaffold); data-contract violations; an entity the running app refuses to create for every schema-valid request (`create contract:`); ruff `F811`/`F821`/`F822`/`F823`; `tsc`/`cargo`/`kotlinc` errors | Drives the loop. Up to 5 rounds of (fix turns → re-validate); stops at zero, gives up after 2 no-progress rounds, rolls back to the pre-Phase-3 snapshot if blockers get worse |
