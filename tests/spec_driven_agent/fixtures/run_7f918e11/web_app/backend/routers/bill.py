...
async def create_bill(bill_data: BillCreate, database: Session = Depends(get_db)) -> Bill:
    ...
    db_bill = Bill(
        issuedDate=bill_data.issuedDate,        settled=bill_data.settled,        totalAmountDue=bill_data.totalAmountDue,        booking_id=bill_data.booking        )
...
async def bulk_create_bill(items: list[BillCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_bill = Bill(
                issuedDate=item_data.issuedDate,                settled=item_data.settled,                totalAmountDue=item_data.totalAmountDue,                booking_id=item_data.booking            )
            database.add(db_bill)
