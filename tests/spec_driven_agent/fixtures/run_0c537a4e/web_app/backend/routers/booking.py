...
def get_all_booking(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    ...
    if detailed:
        ...
        for booking_item in booking_list:
            ...
            bookingroom_list = database.query(BookingRoom).filter(BookingRoom.booking_id == booking_item.id).all()
            item_dict['bookingRooms'] = []
            for bookingroom_obj in bookingroom_list:
                ...
                item_dict['bookingRooms'].append(bookingroom_dict)
...
def get_paginated_booking(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    ...
    for booking_item in booking_list:
        ...
        bookingRooms_ids = database.query(BookingRoom.id).filter(BookingRoom.booking_id == booking_item.id).all()
        item_data = {
...
async def create_booking(booking_data: BookingCreate, database: Session = Depends(get_db)) -> Booking:
    ...
    db_booking = Booking(
        arrivalDate=booking_data.arrivalDate,        totalPrice=booking_data.totalPrice,        commercialStatus=booking_data.commercialStatus.value,        departureDate=booking_data.departureDate,        physicalStatus=booking_data.physicalStatus.value,        employee_id=booking_data.employee,        contact_id=booking_data.contact        )

    database.add(db_booking)
    database.flush()
    database.refresh(db_booking)

    if booking_data.bookingRooms:
        # Validate that all BookingRoom IDs exist
        for bookingroom_id in booking_data.bookingRooms:
        ...
        database.query(BookingRoom).filter(BookingRoom.id.in_(booking_data.bookingRooms)).update(
            {BookingRoom.booking_id: db_booking.id}, synchronize_session=False
        )
        database.flush()
...
async def bulk_create_booking(items: list[BookingCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_booking = Booking(
                arrivalDate=item_data.arrivalDate,                totalPrice=item_data.totalPrice,                commercialStatus=item_data.commercialStatus.value,                departureDate=item_data.departureDate,                physicalStatus=item_data.physicalStatus.value,                employee_id=item_data.employee,                contact_id=item_data.contact            )
            database.add(db_booking)
...
async def update_booking(booking_id: int, booking_data: BookingCreate, database: Session = Depends(get_db)) -> Booking:
    ...
    if booking_data.bookingRooms is not None:
        requested_bookingRooms = set(booking_data.bookingRooms)
        current_bookingRooms = {
            getattr(item, 'id')
            for item in database.query(BookingRoom).filter(BookingRoom.booking_id == db_booking.id).all()
        }
        bookingRooms_to_detach = current_bookingRooms - requested_bookingRooms
        if bookingRooms_to_detach:
            raise HTTPException(status_code=409, detail="BookingRoom " + ", ".join(str(item) for item in sorted(bookingRooms_to_detach)) + " requires a booking: reassign it instead of removing it")
        bookingRooms_to_attach = requested_bookingRooms - current_bookingRooms
        for bookingroom_id in bookingRooms_to_attach:
            db_bookingroom = database.query(BookingRoom).filter(BookingRoom.id == bookingroom_id).first()
            if not db_bookingroom:
                raise HTTPException(status_code=400, detail="BookingRoom with id " + str(bookingroom_id) + " not found")
        if bookingRooms_to_attach:
            database.query(BookingRoom).filter(BookingRoom.id.in_(bookingRooms_to_attach)).update(
                {BookingRoom.booking_id: db_booking.id}, synchronize_session=False
            )
...
async def delete_booking(booking_id: int, database: Session = Depends(get_db)):
    ...
    if db_booking is None:
        raise HTTPException(status_code=404, detail="Booking not found")
    related_bookingRooms = db_booking.bookingRooms
    for related in (list(related_bookingRooms) if isinstance(related_bookingRooms, list) else [item for item in [related_bookingRooms] if item is not None]):
        remaining = 0
        if remaining < 1:
            raise HTTPException(status_code=409, detail="Cannot delete Booking " + str(booking_id) + ": BookingRoom " + str(getattr(related, 'id')) + " requires at least 1 booking")
...
@router.get("/booking/{booking_id}/bookingRooms/", response_model=None, tags=["Booking Relationships"])
async def get_bookingRooms_of_booking(booking_id: int, database: Session = Depends(get_db)):
    """Get all BookingRoom entities related to this Booking through bookingRooms"""
    db_booking = database.query(Booking).filter(Booking.id == booking_id).first()
    if db_booking is None:
        raise HTTPException(status_code=404, detail="Booking not found")

    bookingRooms_list = database.query(BookingRoom).filter(BookingRoom.booking_id == booking_id).all()

    return {
        "booking_id": booking_id,
        "bookingRooms_count": len(bookingRooms_list),
        "bookingRooms": bookingRooms_list
    }
