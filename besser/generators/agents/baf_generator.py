...
class BAFGenerator(GeneratorInterface):
    ...
    """
    def __init__(self, model: StateMachine, output_dir: str = None):
    ...
    def generate(self):
        ...
        bot_template = env.get_template('baf_bot_template.py.j2')
        bot_path = self.build_generation_path(file_name=f"{self.model.name}.py")
        with open(bot_path, mode="w") as f:
            generated_code = bot_template.render(sm=self.model)
