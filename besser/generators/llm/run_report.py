...
``besser/generators/llm/orchestrator.py``) but never emit their own
...
Importable::
    ...
    from besser.generators.llm.run_report import build_report, format_report
    report = build_report(run_dir)   # plain, JSON-serializable dict
    print(format_report(report))

Runnable::

    python -m besser.generators.llm.run_report <run_dir>
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m besser.generators.llm.run_report",
