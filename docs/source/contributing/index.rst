Contributing
======================

We highly value and welcome contributions from the community to enhance and improve the platform. Before opening
an issue or pull request, read our `CONTRIBUTING policy <https://github.com/BESSER-PEARL/BESSER/blob/master/CONTRIBUTING.md>`_
so you understand the review workflow, coding standards, and communication channels.

Use the guides in this section to discover the architecture, pick the area that matches your interests, and follow
repeatable steps when extending BESSER (from the B-UML language to the code generators and the web modeling editor).

.. toctree::
   :maxdepth: 1

   areas
   ../contributor_guide
   ../ai_assistant_guide
   create_generator
   create_dsl
   diagram_dsl_workflow

Our Team
--------

Maintainers:

* `Ivan Alfonso <https://github.com/ivan-alfonso>`_
* `Armen Sulejmani <https://github.com/ArmenSl>`_
* `Jordi Cabot <https://github.com/jcabot>`_

We want to recognize the work of the contributors:

* `Aaron Conrardy <https://github.com/Aran30>`_
* `Fitash Ul-Haq <https://github.com/FitashUlHaq>`_
* `Marcos Gomez <https://github.com/mgv99>`_
* `Atefeh Nirumand <https://github.com/AtefehNirumandJazi>`_
* `Nadia Daoudi <https://github.com/DaoudiNadia>`_
* `Fedor Chikhachev <https://github.com/FChikh>`_
* `Alisa Vorokhta <https://github.com/Vorokhalice>`_

Contact
-------

You can reach us at: info@besser-pearl.org

Website: https://besser-pearl.github.io/website/
